...
/** A linked card. */
export default function CardLink({ d, cids, styles }: { d: CardLinkData; cids: string[]; styles: CardLinkStyles }) {
  return (
    <a data-cid={cids[0]} className="h-18 block p-3 rounded-lg cursor-pointer hover:bg-border" data-component="link" href={d.href}>
      <div data-cid={cids[1]} className="h-12 flex items-center gap-3">
        ...
        <div data-cid={cids[3]} className={cn("block min-w-0 flex-1 max-md:w-36.5 md:max-lg:w-[6.6625rem] 2xl:w-[17.0375rem]", styles.className)}>
          ...
          <p data-cid={cids[6]} className={cn("h-8 overflow-hidden text-muted-foreground text-[0.75rem] line-clamp-2", styles.className4)}>
            {d.description}
          </p>
        </div>
